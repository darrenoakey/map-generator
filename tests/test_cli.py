...
@pytest.mark.network
def test_generate_warrawee_viewport_includes_real_buildings(tmp_path):
    """End-to-end proof of viewport centering + buildings together.

    A tight --radius-m around the camera viewport must produce an STL with
    real extruded buildings. The viewport is deliberately authoritative even
    when the URL also carries a different place-pin marker.
    """
    ...
    assert sidecar["center_lat"] == pytest.approx(-33.7368286, abs=0.0001)
    assert sidecar["center_lon"] == pytest.approx(151.1084364, abs=0.0001)
