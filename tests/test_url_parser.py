...
def test_camera_viewport_used_even_with_a_place_pin_present():
    """The camera viewport is authoritative — it's what the user was actually
    looking at when they grabbed the URL. A place-pin marker (from a search
    result) can point elsewhere and must NOT override it.
    """
    url = (
    ...
    assert extent.center_lat == pytest.approx(10.0, abs=1e-9)
    assert extent.center_lon == pytest.approx(20.0, abs=1e-9)
