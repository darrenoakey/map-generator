...
def parse_google_maps_url(url: str) -> MapExtent:
    ...
    # The `@lat,lon,altm` segment is the camera viewport — what the user was
    # actually looking at when they grabbed the URL. A place URL also carries
    # a `!3d<lat>!4d<lon>` marker for the pinned search result, which can be
    # a different location (e.g. the address centroid vs. the framed view);
    # the viewport is authoritative, not the marker.
    if unit == "m":
        altitude_m = value
    else:
