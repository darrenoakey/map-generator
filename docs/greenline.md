# greenline — map-generator

This repo uses **greenline**: a local, serialized, gated CI/CD tool. There is one
canonical checkout of `map-generator`; it is always on `main`, always clean,
and always identical to what prod (`map-generator`) runs. Every change reaches
...
```
greenline worktree feature-x        # new worktree at /Volumes/Gumby/worktrees/greenline/map-generator/feature-x
cd /Volumes/Gumby/worktrees/greenline/map-generator/feature-x         # branch gl/feature-x off last-green
# ...edit, test, commit...
...
- **`./run deploy`** — cwd = the canonical checkout. Rebuild/restart prod (e.g.
  `auto -q restart map-generator`), **health-check**, exit nonzero on unhealthy, and be
